...
export default function Home() {
  ...
  const handleDelete = () => {
    ...
    deleteMeeting(selectedRows)
      .then((res) => {
        if (res.data) {
          dispatch(
            setMeetings(lists.filter((item) => !selectedRows.includes(item.id)))
          );
        }
      })
      .finally(() => {
  ...
  const getLists = (id) => {
    ...
    getMeetings(id)
      .then((res) => {
        if (res && res.data) dispatch(setMeetings(res?.data));
      })
      .finally(() => {
        dispatch(setLoading(false));
      });
    ...
    <Container maxW="container.2xl" centerContent sx={{ mt: "6rem" }}>
      ...
      <Card w="100%">
                ...
                {filterData
                  ...
                  .map((row, index) => (
                    <Tr cursor="pointer" key={`tbody-row-${index}`}>
                      <Td>
                        <Checkbox
                          isChecked={selectedRows.indexOf(row.id) > -1}
                          onChange={(e) => handleCheckRow(e, row.id)}
                        />
                      </Td>
                      <Td>
                        <Text onClick={() => gotoDetail(row.id)} color="blue">
                          {row.title}
                        </Text>
                      </Td>
                      <Td>
        ...
        <CardFooter display="flex" justify="space-between" alignItems="center">
          <Text>
            Showing {filterData.length ? page * rowsPerPage + 1 : 0} to{" "}
            {page * rowsPerPage + rowsPerPage < filterData.length
          ...
          <HStack>
            ...
            <Button
              colorScheme="blue"
              onClick={nextPage}
              isDisabled={page * rowsPerPage + rowsPerPage >= filterData.length}
            >
              Next
            </Button>
          </HStack>
